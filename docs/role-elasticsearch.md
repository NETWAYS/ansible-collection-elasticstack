...
* *elasticsearch_check_calculation*: When set to `true`, the role runs **only** the node role validation checks and then immediately stops execution on each host via `meta: end_host`. 
