...
DOCUMENTATION = '''
  name: to_datetime_locale
  short_description: Convert datestring with custom defined locale
  description:
    - Converts date strings by custom defined locale to datetime
  options:
    _input:
...
EXAMPLES = '''
- name: Convert 'Mär 16 15:55:01 2026' with to_datetime_locale()
  debug:
    msg: "{{ 'Mär 16 15:55:01 2026' | to_datetime_locale('%b %d %H:%M:%S %Y', 'de_DE', 'UTF-8') }}"
- name: Convert 'Mar 16 15:55:01 2026' with to_datetime_locale()
  debug:
    msg: "{{ 'Mär 16 15:55:01 2026' | to_datetime_locale('%b %d %H:%M:%S %Y') }}"
'''
...
class FilterModule(object):
    def filters(self):
        return {
            'to_datetime_locale': self.to_datetime_locale
        }

    def to_datetime_locale(
            self,
            string,
            format="%Y-%m-%d %H:%M:%S",
        ...
        try:
            ...
            date_time = datetime.datetime.strptime(string, format)
        except Exception as e:
            raise AnsibleFilterError("Exception: %s" % to_native(e))
        return to_text(date_time)
