...
| `elasticsearch_check_calculation` | `bool` | `false` | — | Only run the node-role validation (derive master/data/other, group nodes, assert an odd number of master-eligible nodes) and then stop on each host. Elasticsearch is not installed or configured. Useful as a pre-flight inventory check. |
